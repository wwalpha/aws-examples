...
resource "aws_lakeformation_permissions" "this" {
  ...
  principal   = aws_iam_role.lambda_bucket_event.arn

  table_with_columns {
    database_name = aws_glue_catalog_table.config_snapshots.database_name
    name          = aws_glue_catalog_table.config_snapshots.name
    wildcard      = true
  }
}
