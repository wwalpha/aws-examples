...
- **説明 (必須)**: 変数 (`variable`) と出力 (`output`) には、その目的を説明する `description` を**必ず**記述してください。省略は許可されません。
...
- **リソースコメント (必須)**: 全てのリソース定義（`resource`, `data`, `module`）の直前に、以下のフォーマットでセクションヘッダーコメントを**必ず**記述してください。
  ```hcl
  # ----------------------------------------------------------------------------------------------
